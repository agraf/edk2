...
{
  // TODO check if image can be supported (e.g, boot time or runtime driver, etc)
...
STATIC EDKII_X64_INTERPRETER_PROTOCOL mX86InterpreterProtocol = {
  IsX86ImageSupported,
  RegisterX86Image,
  UnregisterX86Image
};
  ...
  )
{
  ...
  Status = gBS->AllocatePages (AllocateAnyPages, EfiBootServicesCode,
                  EFI_SIZE_TO_PAGES ((32 * 1024 + 1) * 1024), &Alloc);
  if (EFI_ERROR (Status)) {
    return Status;
  }

  code_gen_prologue = (UINT8 *)(UINTN)Alloc;
  static_code_gen_buffer = code_gen_prologue + 1024;
  ...
  Status = gBS->InstallProtocolInterface (&ImageHandle,
                  &gEdkiiX64InterpreterProtocolGuid,
                  EFI_NATIVE_INTERFACE,
                  &mX86InterpreterProtocol);
  if (EFI_ERROR (Status)) {
